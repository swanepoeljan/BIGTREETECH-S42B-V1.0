    ...
    if (enmode == 1)
    {
      if (closemode == 1)
      {
        ...
        if (iterm > UMAXSUM)
          ...
          iterm = -UMAXSUM;
#else
        if (iterm > (currentLimit * 128)) //积分饱和限制
          iterm = currentLimit * 128;
        else if (iterm < -(currentLimit * 128))
          iterm = -(currentLimit * 128);
#endif

        dterm = LPFA * dterm / 128 - LPFB * kd * (yw - yw_1) / 8;
        u = (kp * pid_error + iterm + dterm) / 128; //PID
