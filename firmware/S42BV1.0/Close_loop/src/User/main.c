...
uint16_t tblParams[15]; // vc: Current parameters
...
int main(void)
{
  ...
  FlashUnlock();
  Calibration_flag = FlashReadHalfWord(DATA_STORE_ADDR);
  FlashLock();
  ...
  if (Calibration_flag == 0xAA)
  {
    ...
    // Read parameters
    STMFLASH_Read(DATA_STORE_ADDR, tblParams, sizeof(tblParams));
    ...
    stepangle = tblParams[3];
    Motor_ENmode_flag = tblParams[5];
    Motor_Dir = tblParams[7];
...
void ParseBytes(uint8_t data)
{
  ...
  if (parseState == (uint8_t)PARSE_STATUS_NEW_PACKET)
  {
    ...
    case SERIAL_MSG_READVALUE:
      if (parseBuffer[2] == SERIAL_MSG_READVALUE_SOURCE_STATUS)
      {
        ...
        uint16_t len = Serial_GeneratePacket(SERIAL_MSG_ANGLE, (uint8_t *)&status, sizeof(status));
      ...
      if (parseBuffer[2] == SERIAL_MSG_READVALUE_SOURCE_ANGLE)
      {
        ...
        uint16_t len = Serial_GeneratePacket(SERIAL_MSG_ANGLE, (uint8_t *)&aa, sizeof(aa));
        UART1_Write(packetBuffer, len);
      }
      if (parseBuffer[2] == SERIAL_MSG_READVALUE_SOURCE_ANGERROR)
      {
        ...
        uint16_t len = Serial_GeneratePacket(SERIAL_MSG_ANGERROR, (uint8_t *)&ae, sizeof(ae));
        UART1_Write(packetBuffer, len);
      }
      break;
    ...
    case SERIAL_MSG_GETPARAM:
    {
      ...
      if (parseBuffer[2] == SERIAL_MSG_PARAM_SOURCE_KP)
        len = Serial_GeneratePacket(SERIAL_MSG_PARAM_KP, (uint8_t *)&kp, sizeof(kp));
      else if (parseBuffer[2] == SERIAL_MSG_PARAM_SOURCE_KI)
        len = Serial_GeneratePacket(SERIAL_MSG_PARAM_KI, (uint8_t *)&ki, sizeof(ki));
      else if (parseBuffer[2] == SERIAL_MSG_PARAM_SOURCE_KD)
        len = Serial_GeneratePacket(SERIAL_MSG_PARAM_KD, (uint8_t *)&kd, sizeof(kd));
      else if (parseBuffer[2] == SERIAL_MSG_PARAM_SOURCE_CURRENT)
        len = Serial_GeneratePacket(SERIAL_MSG_PARAM_CURRENT, (uint8_t *)&currentLimit, sizeof(currentLimit));
      else if (parseBuffer[2] == SERIAL_MSG_PARAM_SOURCE_STEPSIZE)
        len = Serial_GeneratePacket(SERIAL_MSG_PARAM_STEPSIZE, (uint8_t *)&stepangle, sizeof(stepangle));
      else if (parseBuffer[2] == SERIAL_MSG_PARAM_SOURCE_ENDIR)
        len = Serial_GeneratePacket(SERIAL_MSG_PARAM_ENDIR, (uint8_t *)&Motor_ENmode_flag, sizeof(Motor_ENmode_flag));
      else if (parseBuffer[2] == SERIAL_MSG_PARAM_SOURCE_MOTORDIR)
        len = Serial_GeneratePacket(SERIAL_MSG_PARAM_MOTORDIR, (uint8_t *)&Motor_Dir, sizeof(Motor_Dir));
      else
      {
        // Parameter not found
      }
    ...
    case SERIAL_MSG_SETPARAM:
      ...
      if (parseBuffer[2] == SERIAL_MSG_PARAM_SOURCE_CURRENT)
        currentLimit = (int16_t)((parseBuffer[3] << 8) | parseBuffer[4]);
